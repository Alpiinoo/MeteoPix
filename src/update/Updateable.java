    ...
    public default Updateable isColliding(Renderable thisObject, String otherObjectID){
        ...
        for (Updateable object: objects)
            if(object.getID() == otherObjectID)
                if(thisObject.getX() < object.getRenderable().getX() + object.getRenderable().getWidth() && thisObject.getX() +
                thisObject.getWidth() > object.getRenderable().getX())
                    if(thisObject.getY() < object.getRenderable().getY() + object.getRenderable().getHeight() && thisObject.getY() +
                    thisObject.getHeight() > object.getRenderable().getY())
                        return object;
        return null;
    }
}
